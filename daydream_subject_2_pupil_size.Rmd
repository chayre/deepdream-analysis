...
create_pupil_plot <- function(data, title) {
  ggplot(data, aes(x = ExperimentSecond - ExperimentSecond[1])) +
    geom_smooth(aes(y = round(LeftEyePupilSize, 3), color = "Left Pupil Size"), se = FALSE, method = "loess", span = 0.05) +
    geom_smooth(aes(y = round(RightEyePupilSize, 3), color = "Right Pupil Size"), se = FALSE, method = "loess", span = 0.05) +
    geom_line(aes(y = Luminosity * 14, color = "Luminosity")) + 
    scale_y_continuous(
      name = "Pupil Size (mm)",
      sec.axis = sec_axis(~ . / 14, name = "Luminosity")
    ) +
    theme_minimal() +
    labs(
      title = title,
    ...
    scale_color_manual(
      values = c("Left Pupil Size" = "red", "Right Pupil Size" = "black", "Luminosity" = adjustcolor( "green", alpha.f = 0.5)))
}

# Create individual plots for the four conditions
plot1 <- create_pupil_plot(
  data_interpolated %>% 
    filter(data_interpolated$VideoMode == "Normal", data_interpolated$VideoName == "Main_Mono_HighLayer_Strong", data_interpolated$Phase == "Video"), "Normal Mono")
plot2 <- create_pupil_plot(
  data_interpolated %>% 
    filter(data_interpolated$VideoMode == "Normal", data_interpolated$VideoName == "Main_Stereo_HighLayerStrong", data_interpolated$Phase == "Video"), "Normal Stereo")
plot3 <- create_pupil_plot(
  data_interpolated %>% 
    filter(data_interpolated$VideoMode == "Hallucinations", data_interpolated$VideoName == "Main_Mono_HighLayer_Strong", data_interpolated$Phase == "Video"), "Hallucination Mono")
plot4 <- create_pupil_plot(
  data_interpolated %>% 
    filter(data_interpolated$VideoMode == "Hallucinations", data_interpolated$VideoName == "Main_Stereo_HighLayerStrong", data_interpolated$Phase == "Video"), "Hallucination Stereo")

lumtest <- data_interpolated %>% 
