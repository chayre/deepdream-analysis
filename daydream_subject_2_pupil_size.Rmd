...
# Linear model
model <- lm((LeftEyePupilSize + RightEyePupilSize) / 2 ~ InverseLuminosity, data = filtered_data)

# Non-linear model (generalized additive model)
fit_gam_model <- function(data) {
  gam((LeftEyePupilSize + RightEyePupilSize) / 2 ~ s(InverseLuminosity), data = data)
}

# Create the model
model_gam <- fit_gam_model(data_interpolated)

# Normalization
data_subtractive <- normalize_pupil_size(filtered_data, model)
...
p <- (plot1 + plot2) / (plot3 + plot4) +
  plot_layout(guides = "collect") + # Collect the legends into one
  plot_annotation(title = "Subject 2 Pupil Size Normalized by Luminosity") # Add a supertitle
