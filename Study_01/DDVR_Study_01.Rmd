...
# Load and combine files, fixing AnswerDuration and Timestamp
df_full <- list.files(path = dir, 
                      pattern = "_QuestionsData.csv", 
                      full.names = TRUE) %>%
  map_dfr(~ {
    df <- read.csv(.x, sep = ";", header = TRUE)  # Read the file
    if ("AnswerDuration" %in% names(df)) {         # Fix AnswerDuration to numeric
      df$AnswerDuration <- as.numeric(as.character(df$AnswerDuration))
    }
    if ("Timestamp" %in% names(df)) {              # Ensure Timestamp is character
      df$Timestamp <- as.character(df$Timestamp)
    }
    if ("Sex" %in% names(df)) {                    # Ensure Sex is character
      df$Sex <- as.character(df$Sex)
    }
    if ("VideoName" %in% names(df)) {              # Ensure VideoName is character
      df$VideoName <- as.character(df$VideoName)
    }
    if ("VideoMode" %in% names(df)) {              # Ensure VideoMode is character
      df$VideoMode <- as.character(df$VideoName)
    }
    df
  })
...
# First plot: Weak condition vs. Strong Condition in Hallucination
df_lowlayer_individuals <- df %>%
  filter(VideoMode == "LowLayer") %>%
  filter(QuestionIndex %in% as.factor(13:1)) %>%
  mutate(VideoName = factor(VideoName, levels = c("Weak", "Strong")))

p_lowlayer_individuals <- ggplot(df_lowlayer_individuals, 
  ...
  facet_wrap(~ Id + VideoName, ncol = 2) +
  theme_minimal() +
  scale_y_continuous(limits = c(0, 100)) +
  labs(
    title = "LowLayer Hallucinations Condition: Weak vs. Strong",
    x = "Question Index",
    y = "Response")

# Second plot: Hallucinations condition - separate plots for each subject
df_midlayer_individuals <- df %>%
  filter(VideoMode == "MidLayer") %>%
  filter(QuestionIndex %in% as.factor(13:1)) %>%
  mutate(VideoName = factor(VideoName, levels = c("Weak", "Strong")))

p_midlayer_individuals <- ggplot(df_midlayer_individuals, 
    aes(x = QuestionIndex, y = QuestionAnswer, fill = QuestionIndex)) +
  geom_bar(stat = "identity", position = position_dodge(), alpha = 0.6) +
  scale_fill_manual(values = question_colors) +
  facet_wrap(~ Id + VideoName, ncol = 2) +
  ...
  scale_y_continuous(limits = c(0, 100)) +
  labs(
    title = "MidLayer Hallucinations Condition: Weak vs. Strong",
    x = "Question Index",
    y = "Response")

# Third plot: Hallucinations condition - separate plots for each subject
df_highlayer_individuals <- df %>%
  filter(VideoMode == "HighLayer") %>%
  filter(QuestionIndex %in% as.factor(13:1)) %>%
  mutate(VideoName = factor(VideoName, levels = c("Weak", "Strong")))

p_highlayer_individuals <- ggplot(df_highlayer_individuals, 
    aes(x = QuestionIndex, y = QuestionAnswer, fill = QuestionIndex)) +
  geom_bar(stat = "identity", position = position_dodge(), alpha = 0.6) +
  scale_fill_manual(values = question_colors) +
  facet_wrap(~ Id + VideoName, ncol = 2) +
  ...
  scale_y_continuous(limits = c(0, 100)) +
  labs(
    title = "HighLayer Hallucinations Condition: Weak vs. Strong",
    x = "Question Index",
    y = "Response"
  )

# Arrange plots side by side
#grid.arrange(p_lowlayer_individuals, p_midlayer_individuals, p_highlayer_individuals, ncol = 2)
plot(p_lowlayer_individuals)
