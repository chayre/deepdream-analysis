...
# Print all plots
walk(names(meanhr_plots_ttest), ~print(meanhr_plots[[.x]]))
walk(names(hrv_plots_ttest), ~print(hrv_plots[[.x]]))
...
# 50% average threshold to identify outliers
analyze_metric_outliers <- function(data, metric_col) {
  data %>%
    group_by(Id, Condition_Order) %>%
    mutate(
      avg_val = mean(!!sym(metric_col), na.rm = TRUE),
      lower_threshold = 0.5 * avg_val,  # 50% below average
      upper_threshold = 1.5 * avg_val,  # 50% above average
...
## Re-plotting HR and HRV without Outliers
...
filtered_exp_cleaned <- filtered_exp_synced %>%
  ...
  mutate(
    ...
    lower_threshold_rr = 0.5 * avg_rr,
    upper_threshold_rr = 1.5 * avg_rr,
    ...
    lower_threshold_hr = 0.5 * avg_hr,
    upper_threshold_hr = 1.5 * avg_hr,
...
# Print all plots
walk(names(meanhr_plots_no_outlier), ~print(meanhr_plots_no_outlier[[.x]]))
walk(names(hrv_plots_no_outliers), ~print(hrv_plots_no_outliers[[.x]]))

# Create PDF report
pdf("HR_HRV_Report.pdf", width = 11, height = 8.5)  # Letter size landscape
#plot.new()
#print("Results")
#grid.table(linked_results)
#print("Results without Outliers")
#grid.table(linked_results_filtered)
#print("T-test Results")
#grid.table(paired_ttest_results)
#print("T-test Results without Outliers")
#grid.table(paired_ttest_results_filtered)
