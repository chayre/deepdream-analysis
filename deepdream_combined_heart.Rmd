...
```
## Plot R-R For All Subjects/Conditions
...
# Plotting function
create_subject_plot <- function(subject_data) {
...
subject_plots <- filtered_exp_synced %>%
  group_split(Id) %>%
  map(create_subject_plot)

# View all plots
walk(subject_plots, print) 
...
mad_score <- function(x, threshold = 3.5) {
  median_x <- median(x)
  mad_x <- mad(x)
  z_scores <- abs(x - median_x) / mad_x
  return(z_scores > threshold)
}

# Add MAD outliers and thresholds to data
filtered_exp_annotated <- filtered_exp_synced %>%
  group_by(Id, Condition_Order) %>%
  mutate(
    median_rr = median(`Clean R-R`, na.rm = TRUE),
    mad_threshold = 3.5 * mad(`Clean R-R`, na.rm = TRUE),
    is_outlier = mad_score(`Clean R-R`, threshold = 3.5)
  ) %>%
  ungroup()

# Modified plotting function with outlier visualization
create_subject_plot <- function(subject_data) {
  # Get outlier counts per condition
  ...
  ggplot(subject_data, aes(x = Time, y = `Clean R-R`)) +
    ...
    geom_point(data = ~ filter(.x, is_outlier), 
               color = "red", size = 0.8, alpha = 0.7) +
    geom_hline(aes(yintercept = median_rr), 
               color = "blue", linetype = "dashed", linewidth = 0.3) +
    geom_hline(aes(yintercept = median_rr + mad_threshold), 
    ...
    geom_text(data = outlier_counts,
              ...
              hjust = -0.1, vjust = 1.5, size = 2.5, color = "red") +
    facet_wrap(~ Condition_Order, nrow = 2, ncol = 2, scales = "free_x") +
    labs(
      title = paste("Subject", unique(subject_data$Id)),
      x = "Time (s)",
      y = "R-R Interval (ms)",
      caption = "Blue dashed: Median | Green dotted: MAD thresholds"
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 10, hjust = 0.5),
...
# Create and save all plots with outliers marked
subject_plots <- filtered_exp_annotated %>%
  group_split(Id) %>%
  map(create_subject_plot)

# Print all plots (use arrow buttons in RStudio to navigate)
walk(subject_plots, print)

# Get tabular summary of outlier counts
