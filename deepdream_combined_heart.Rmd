...
```{r warning=FALSE, message=FALSE}
# Import ECG data
import_subjects_exp <- function(data_dir = "C:\\Users\\CAyre\\Documents\\Coding\\deepdream-analysis\\Study_01\\ECG\\ACQ") {
...
import_subjects_base <- function(data_dir = "C:\\Users\\CAyre\\Documents\\Coding\\deepdream-analysis\\Study_01\\ECG\\ACQ") {
  # Get list of all subject files (e.g., ID01_baseline.csv, ID02_baseline.csv)
  file_list <- list.files(
    path = here(data_dir),
...
unique_combinations <- df_sorted %>%
  distinct(section_id, Id)

for (i in 1:nrow(unique_combinations)) {
  section_val <- unique_combinations$section_id[i]
  subject_val <- unique_combinations$Id[i]

  # Filter data for the specific section and subject
  df_subset <- df_sorted %>%
  ...
  python_results <- rbind(python_results, data.frame(
    section_id = section_val,
    Id = subject_val,
    HR_entropy = entropy_result[2]  # Second element is entropy value
  ))
}
...
data_base_processed_no_outliers <- data_base_processed %>%
  group_by(Id) %>%
  mutate(
    # HRV (R-R) calculations
    avg_rr = mean(`Clean R-R`, na.rm = TRUE),
...
# Import CSV files and combine them into a single data frame
device_combined <- bind_rows(lapply(file_paths, read_csv2))
